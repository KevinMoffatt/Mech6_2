...
Servo armServo;
int initialArmPos = 47; //starting servo angle
int wallAngle = 90; //servo angle for wall
int railAngle = 130; //servo angle to reach rail runner
int warpedAngle = 120; //servo angle to hook onto warped wall
...
void lineFollow() { //line following
  ...
  else if (sensorBias[0] < 1200 && sensorBias[1] < 1000 && sensorBias[2] < 1000 && sensorBias[3] < 1000 && sensorBias[4] < 1000 && sensorBias[5] < 1000 && sensorBias[6] < 1000 && sensorBias[7] < 1000) { //if all sensors detect white
    md.setM1Brake(400);
    md.setM2Brake(400);
...
void railRun() { //railrunner code
  ...
  delay(6500); //give the jack time to extend
  ...
  md.setM3Speed(0);
  md.setM4Speed(0);
  md.setM4Speed(-400); //lower the jack
  delay(4000); //give the jack time to extend
  md.setM4Brake(0);  //stop the jack
  railRunCheck = 1;
}

void warpedWall() {
  md.setM4Speed(400);//drive jack up
  delay(5000);//give time for the jack to extend
